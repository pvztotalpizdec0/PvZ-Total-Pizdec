...
LawnApp::LawnApp()
{
	...
	mProdName = "io.github.pvztotalpizdec.pvz-totalpizdec";
	std::string aTitleName = "PvZ Total Pizdec";
	mTitle = aTitleName;
...
void LawnApp::DoConfirmBackToMain()
{
	LawnDialog* aDialog = (LawnDialog*)DoDialog(
		...
		true, 
		GetString("LEAVE_GAME_HEADER", "[LEAVE_GAME_HEADER]"),
	...
	aDialog->mLawnNoButton->mLabel = TodStringTranslate("[S_DIALOG_BUTTON_CANCEL]");
...
void LawnApp::DoPauseDialog()
{
	...
	LawnDialog* aDialog = (LawnDialog*)DoDialog(
		...
		true,
		GetString("GAME_PAUSED", "[GAME_PAUSED]"),
		GetString("CLICK_TO_RESUME", "[CLICK_TO_RESUME]"),
		GetString("RESUME_GAME", "[RESUME_GAME]"), 
...
void LawnApp::FinishCreateUserDialog(bool isYes)
{
	...
	if (isYes && aName.empty())
	{
		DoDialog(
			...
			true,
			GetString("ENTER_YOUR_NAME", "[ENTER_YOUR_NAME]"),
	...
	else
	{
		...
		if (aProfile == nullptr)
		{
			DoDialog(
				Dialogs::DIALOG_CREATEUSERERROR,
				true,
				GetString("NAME_CONFLICT", "[NAME_CONFLICT]"),
...
void LawnApp::DoConfirmDeleteUserDialog(const std::string& theName)
{
	...
	DoDialog(
		...
		true, 
		GetString("ARE_YOU_SURE", "[ARE_YOU_SURE]"), 
		StrFormat(
			GetString("DELETE_USER_WARNING", "[DELETE_USER_WARNING]").c_str(),
			theName.c_str()),
		"", 
		Dialog::BUTTONS_YES_NO
	);
...
//#endif
	...
	mSunCheck = new TypingCheck("sun");
	mFastCheck = new TypingCheck("fast");
	mLazyCheck = new TypingCheck("lazy");
...
void LawnApp::URLOpenFailed(const std::string& theURL)
{
	...
	DoDialog(Dialogs::DIALOG_OPENURL_WAIT, true, GetString("OPEN_BROWSER", "Open Browser"), "[S_DIALOG_BUTTON_OK]", aString, Dialog::BUTTONS_FOOTER);
}
...
void LawnApp::DrawCrazyDave(Graphics* g)
{
	...
	if (mCrazyDaveMessageText.size())
	{
		...
		if (clickToContinue)
		{
			TodDrawString(g, GetString("CLICK_TO_CONTINUE", "[CLICK_TO_CONTINUE]"), aPosX + 139, aPosY + 140, FONT_PICO129, Color::Black, DrawStringJustification::DS_ALIGN_CENTER);
		}
	}
