...
void Coin::Update()
{
    ...
    if (mApp->mLazyMode && (IsSun() || IsMoney()))
    {
...
Color Coin::GetColor()
{
    if ((IsSun() || IsMoney()) && mIsBeingCollected)
    {
        ...
        return Color(255, 255, (mApp->mSunMode && IsSun()) ? 32 : 225, aAlpha);
    }
    ...
    return Color(255, 255, (mApp->mSunMode && IsSun()) ? 32 : 225, 225);
}
...
{
    return mType == CoinType::COIN_SUN ? (mApp->mSunMode ? 50 : 25) : mType == CoinType::COIN_SMALLSUN ? (mApp->mSunMode ? 25 : 15) : mType == CoinType::COIN_LARGESUN ? (mApp->mSunMode ? 100 : 50) : 0;
}
