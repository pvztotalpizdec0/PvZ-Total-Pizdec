...
void StoreScreen::PurchaseItem(StoreItem theStoreItem)
{
    ...
    if (!CanAffordItem(theStoreItem))
    {
        ...
        Dialog* aDialog = mApp->DoDialog(DIALOG_NOT_ENOUGH_MONEY, true,
            ...
            "[S_DIALOG_BUTTON_OK]", BUTTONS_FOOTER);
        mWaitForDialog = true;
    ...
    else
    {
        ...
        aComfirmDialog->mLawnYesButton->SetLabel("[S_DIALOG_BUTTON_YES]");
        aComfirmDialog->mLawnNoButton->SetLabel("[S_DIALOG_BUTTON_NO]");
        ...
        if (aComfirmResult == ID_OK)
        {
            ...
            if (theStoreItem == STORE_ITEM_PACKET_UPGRADE)
            {
                ...
                Dialog* aDialog = mApp->DoDialog(DIALOG_UPGRADED, true, mApp->GetString("MORE_SLOTS", "More slots!"), aDialogLines, "[S_DIALOG_BUTTON_OK]", BUTTONS_FOOTER);
            ...
            else if (theStoreItem == STORE_ITEM_TREE_OF_WISDOM)
            {
                ...
                aDialog->mLawnYesButton->SetLabel("[S_DIALOG_BUTTON_YES]");
                aDialog->mLawnNoButton->SetLabel("[S_DIALOG_BUTTON_NO]");
...
void StoreScreen::MouseDown(int x, int y, int theClickCount)
{
    ...
    for (int aItemPos = 0; aItemPos < MAX_PAGE_SPOTS; aItemPos++)
    {
        ...
        if (Rect(aItemX, aItemY, 50, 87).Contains(x, y))
        {
            ...
            {
                mWaitForDialog = true;
                mApp->LawnMessageBox(DIALOG_MESSAGE, "[GET_FULL_VERSION_TITLE]", "[FULL_VERSION_TO_BUY]", "[S_DIALOG_BUTTON_OK]", "", BUTTONS_FOOTER);
                mWaitForDialog = false;
            }
