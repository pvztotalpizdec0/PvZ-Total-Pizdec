...
UserDialog::UserDialog(LawnApp* theApp) : LawnDialog(theApp, Dialogs::DIALOG_USERDIALOG, true, theApp->GetString("WHO_ARE_YOU", "[WHO_ARE_YOU]"), "", "", Dialog::BUTTONS_OK_CANCEL)
{
    ...
    mRenameButton = MakeButton(UserDialog::UserDialog_RenameUser, this, mApp->GetString("RENAME_BUTTON", "[RENAME_BUTTON]"));
    mDeleteButton = MakeButton(UserDialog::UserDialog_DeleteUser, this, mApp->GetString("DELETE_BUTTON", "[DELETE_BUTTON]"));
    ...
    if (mNumUsers < 8)
    {
        mUserList->AddLine(mApp->GetString("CREATE_NEW_USER", "[CREATE_NEW_USER]"), false);
    }
