	...
	Dialog(nullptr, nullptr, Dialogs::DIALOG_NEWOPTIONS, true, "Options", "", "", Dialog::BUTTONS_NONE)
{
    ...
    if (mFromGameSelector)
    {
        ...
        mBackToGameButton->SetLabel("[S_DIALOG_BUTTON_OK]");
...
void NewOptionsDialog::Draw(Sexy::Graphics* g)
{
    ...
    TodDrawString(g, mApp->GetString("OPTIONS_MUSIC_LABEL", "[OPTIONS_MUSIC_LABEL]"), aSliderLabelsX, 140 + aMusicOffset, FONT_DWARVENTODCRAFT18, aTextColor, DrawStringJustification::DS_ALIGN_RIGHT);
    TodDrawString(g, mApp->GetString("OPTIONS_SOUNDFX", "[OPTIONS_SOUNDFX]"), aSliderLabelsX, 167 + aSfxOffset, FONT_DWARVENTODCRAFT18, aTextColor, DrawStringJustification::DS_ALIGN_RIGHT);
    TodDrawString(g, mApp->GetString("OPTIONS_3D_ACCELERATION", "[OPTIONS_3D_ACCELERATION]"), aCheckboxLabelsX, 197 + a3DAccelOffset, FONT_DWARVENTODCRAFT18, aTextColor, DrawStringJustification::DS_ALIGN_RIGHT);
    TodDrawString(g, mApp->GetString("OPTIONS_FULL_SCREEN", "[OPTIONS_FULL_SCREEN]"), aCheckboxLabelsX, 229 + aFullScreenOffset, FONT_DWARVENTODCRAFT18, aTextColor, DrawStringJustification::DS_ALIGN_RIGHT);
    if (aFontScale != 1.0f)
...
void NewOptionsDialog::ButtonDepress(int theId)
{
    ...
    case NewOptionsDialog::NewOptionsDialog_Restart:
    {
        if (mApp->mBoard)
        {
            ...
            aDialog->mLawnYesButton->mLabel = mApp->GetString("RESTART_LABEL", "[RESTART_LABEL]");
            aDialog->mLawnNoButton->mLabel = mApp->GetString("S_DIALOG_BUTTON_CANCEL", "[S_DIALOG_BUTTON_CANCEL]");
