...
void GameSelector::KeyDown(KeyCode theKey)
{
	...
	{
		mApp->mSunMode = !mApp->mSunMode;
		return;
	}
	if (mApp->mFastCheck->Check(theKey))
	{
		mApp->mFastMode = !mApp->mFastMode;
		return;
	}
	if (mApp->mLazyCheck->Check(theKey))
	{
		mApp->mLazyMode = !mApp->mLazyMode;
		return;
	}
}
...
void GameSelector::ButtonDepress(int theId)
{
	...
	if (theId == GameSelector::GameSelector_Minigame && mMinigamesLocked)
	{
		mApp->LawnMessageBox(Dialogs::DIALOG_MESSAGE, "[MODE_LOCKED]", "[MINIGAME_LOCKED_MESSAGE]", "[S_DIALOG_BUTTON_OK]", "", Dialog::BUTTONS_FOOTER);
		return;
	}
	if (theId == GameSelector::GameSelector_Puzzle && mPuzzleLocked)
	{
		mApp->LawnMessageBox(Dialogs::DIALOG_MESSAGE, "[MODE_LOCKED]", "[PUZZLE_LOCKED_MESSAGE]", "[S_DIALOG_BUTTON_OK]", "", Dialog::BUTTONS_FOOTER);
		return;
	}
	if (theId == GameSelector::GameSelector_Survival && mSurvivalLocked)
	{
		mApp->LawnMessageBox(Dialogs::DIALOG_MESSAGE, "[MODE_LOCKED]", "[SURVIVAL_LOCKED_MESSAGE]", "[S_DIALOG_BUTTON_OK]", "", Dialog::BUTTONS_FOOTER);
		return;
	}
