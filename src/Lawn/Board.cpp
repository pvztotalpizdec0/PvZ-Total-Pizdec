...
Board::Board(LawnApp* theApp)
{
	...
	mSunMode = mApp->mSunMode;
	mFastMode = mApp->mFastMode;
	mLazyMode = mApp->mLazyMode;
	...
	{
		mMenuButton->SetLabel(mApp->mSpawnRate == 8 ? "[MENU_BUTTON_8X]" : "[MENU_BUTTON]");
...
void Board::PickZombieWaves()
{
	...
	for (int aWave = 0; aWave < mNumWaves; aWave++)
	{
		...
		if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND)
		{
			aZombiePoints = (mChallenge->mSurvivalStage * GetNumWavesPerSurvivalStage() + aWave + 10) * 2 / 5 + mApp->mSpawnRate;
		}
		else if (mApp->IsSurvivalMode() && mChallenge->mSurvivalStage > 0)
		{
			aZombiePoints = (mChallenge->mSurvivalStage * GetNumWavesPerSurvivalStage() + aWave) * 2 / 5 + mApp->mSpawnRate;
		}
		else if (mApp->IsAdventureMode() && mApp->HasFinishedAdventure() && mLevel != 5)
		{
			aZombiePoints = aWave * 2 / 5 + mApp->mSpawnRate;
		}
		else
		{
			aZombiePoints = aWave / 3 + mApp->mSpawnRate;
		}
...
{
	ZombieType aRandomZombieType = (ZombieType)RandRangeInt((int)ZOMBIE_NORMAL, (int)NUM_ZOMBIE_TYPES - 1);

	while (aRandomZombieType == (int)ZombieType::ZOMBIE_BOSS || 
		aRandomZombieType == (int)ZombieType::ZOMBIE_BUNGEE || 
		aRandomZombieType == (int)ZombieType::ZOMBIE_BOBSLED)
	{
		aRandomZombieType = (ZombieType)RandRangeInt((int)ZOMBIE_NORMAL, (int)NUM_ZOMBIE_TYPES - 1);
	}

	TodWeightedArray aZombieWeightArray[(int)ZombieType::NUM_ZOMBIE_TYPES];
	int aCount = 1;

	aZombieWeightArray[0].mItem = aRandomZombieType;
	aZombieWeightArray[0].mWeight = GetZombieDefinition(aRandomZombieType).mPickWeight;

	for (int i = 0; i < aCount; i++)
	{
		ZombieType aZombieType = (ZombieType)aZombieWeightArray[i].mItem;
		const ZombieDefinition& aZombieDef = GetZombieDefinition(aZombieType);
		aZombieWeightArray[i].mWeight = 1;
	}
...
void Board::UpdateToolTip()
{
	...
	{
		int aRefresh = (aSeedPacket->mRefreshTime - aSeedPacket->mRefreshCounter);
		std::string aRefreshString = std::to_string(aRefresh / 100);

		std::string aSecForPacket = TodStringTranslate("[SEC_FOR_PACKET]");
		aSecForPacket = TodReplaceString(aSecForPacket, "{SECONDS}", aRefreshString);
		mToolTip->SetWarningText(aSecForPacket);
	}
	else if (!CanTakeSunMoney(aPlantCost) && !HasConveyorBeltSeedBank() && !mApp->IsSlotMachineLevel())
	{
		int aSuns = (aPlantCost - mSunMoney);
		std::string aSunsString = std::to_string(aSuns);

		std::string aSunsForPacket = TodStringTranslate("[NEED_SUNS_FOR_PACKET]");
		aSunsForPacket = TodReplaceString(aSunsForPacket, "{NEED_SUNS}", aSunsString);
		mToolTip->SetWarningText(aSunsForPacket);
	}
...
void Board::SetSunMode(bool theEnableSun)
{
	mApp->PlayFoley(FoleyType::FOLEY_SUN);
	mSunMode = theEnableSun;
	mApp->mSunMode = theEnableSun;
}

void Board::SetFastMode(bool theEnableFast)
{
	mFastMode = theEnableFast;
	mApp->mFastMode = theEnableFast;
}

void Board::SetLazyMode(bool theEnableLazy)
{
	mLazyMode = theEnableLazy;
	mApp->mLazyMode = theEnableLazy;
}
...
void Board::DoTypingCheck(KeyCode theKey)
{
	...
	if (mApp->mSunCheck->Check(theKey))
	{
		SetSunMode(!mSunMode);
		return;
	}
	if (mApp->mFastCheck->Check(theKey))
	{
		SetFastMode(!mFastMode);
		return;
	}
	if (mApp->mLazyCheck->Check(theKey))
	{
		SetLazyMode(!mLazyMode);
		return;
	}
}
...
{
	bool aCanUseKeybinds = !mPaused || mApp->mGameScene == GameScenes::SCENE_PLAYING || mApp->mCrazyDaveState != CrazyDaveState::CRAZY_DAVE_OFF;
	...
	else if (tolower(theChar) == '`' && aCanUseKeybinds && mShowShovel)
	{
		if (mCursorObject->mCursorType != CursorType::CURSOR_TYPE_SHOVEL)
		{
			if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_PLANT_FROM_BANK)
