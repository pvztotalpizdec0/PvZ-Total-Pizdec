...
void Zombie::ZombieInitialize(int theRow, ZombieType theType, bool theVariant, Zombie* theParentZombie, int theFromWave)
{
    ...
    case ZombieType::ZOMBIE_NEWSPAPER:
    {
        ...
        ReanimatorTrackInstance* aTrackInstance = aBodyReanim->GetTrackInstanceByName("Zombie_paper_paper");
        ...
        aBodyReanim->mFrameBasePose = 0;
        TodScaleRotateTransformMatrix(aAttachEffect->mOffset, 55.0f, 37.5f, 0.2f, -1.0f, 1.0f);
    ...
    case ZombieType::ZOMBIE_DANCER:
        ...
        if (!IsOnBoard())
        {
            PlayZombieReanim("anim_armraise", ReanimLoopType::REANIM_LOOP, 0, 24.0f);
        }
        else
        {
            ...
            PlayZombieReanim("anim_moonwalk", ReanimLoopType::REANIM_LOOP, 0, 48.0f);
        }
        mBodyHealth = 500;
        mVariant = false;
        break;
...
void Zombie::PickRandomSpeed()
{
    ...
    {
        mVelX = RandRangeFloat(1.01f, 1.03f);
    }
    else if (mZombiePhase == ZombiePhase::PHASE_POLEVAULTER_PRE_VAULT)
    {
        mVelX = RandRangeFloat(1.22f, 1.24f);
    }
    ...
    {
        mVelX = RandRangeFloat(1.23f, 1.25f);
    }
    else
    {
...
void Zombie::UpdateZombieNewspaper()
{
    ...
    else if (mPhaseCounter == 29 || mPhaseCounter == 4)
    {
        ...
        float aOriginX = mPosX + aTransform.mTransX - 11.0f;
        float aOriginY = mPosY + aTransform.mTransY + 42.0f;
#ifdef DO_FIX_BUGS
        ...
        if (mPhaseCounter == 4)
        {
            for (int i = 0; i < 2; i++)
            {
                Projectile* aProjectile2 = mBoard->AddProjectile(aOriginX, aOriginY, mRenderOrder, mRow, ProjectileType::PROJECTILE_ZOMBIE_PEA);
...
void Zombie::UpdateZombieBackupDancer()
{
    ...
    if (aDancerPhase != mZombiePhase)
    {
        ...
        case ZombiePhase::PHASE_DANCER_WALK_TO_RAISE:
            mZombiePhase = aDancerPhase;
            PlayZombieReanim("anim_armraise", ReanimLoopType::REANIM_LOOP, 10, 36.0f);
    ...
    while (mBoard->IteratePlants(aPlant))
    {
        if (GetCircleRectOverlap(mPosX, mPosY, 60, aPlant->GetPlantRect()))
        {
            aPlant->mPlantHealth -= DAMAGE_PER_EAT / 2;
    ...
    if (mSummonCounter > 0)
    {
        ...
        if (mSummonCounter == 0)
        {
            if (GetDancerFrame() == 12 && mHasHead && mPosX < 700.0f)
            {
                ...
                PlayZombieReanim("anim_point", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 20, 48.0f);
            }
            else
            {
                mSummonCounter = 1;
...
void Zombie::DropHead(unsigned int theDamageFlags)
{
    ...
    if (mZombieType == ZombieType::ZOMBIE_DUCKY_TUBE)
    {
        int aPosX = mX + mWidth / 2;
        int aPosY = mY + mHeight / 2;
        mBoard->KillAllPlantsInRadius(aPosX, aPosY, 25);
        mApp->PlayFoley(FoleyType::FOLEY_BONK);
    }
}
...
void Zombie::UpdateZamboni()
{
    ...
    if (mPhaseCounter <= 0)
    {
        ...
        while (mBoard->IterateZombies(aZombie))
        {
            ...
            bool aCanGoUp = true;
            bool aCanGoDown = true;
            bool aIsPool = mBoard->mPlantRow[aZombie->mRow] == PlantRowType::PLANTROW_POOL;
            if (!mBoard->RowCanHaveZombies(aZombie->mRow - 1))
            {
                aCanGoUp = false;
            }
            else if (mBoard->mPlantRow[aZombie->mRow - 1] == PlantRowType::PLANTROW_POOL && !aIsPool)
            {
                aCanGoUp = false;
            }
            else if (mBoard->mPlantRow[aZombie->mRow - 1] != PlantRowType::PLANTROW_POOL && aIsPool)
            ...
            {
                aZombie->SetRow((Rand(2) == 0) ? (aZombie->mRow + 1) : (aZombie->mRow - 1));
            }
            else
            {
                TOD_ASSERT(false);
...
void Zombie::AnimateChewSound()
{
    ...
    if (aPlant)
    {
        if (aPlant->mSeedType == SeedType::SEED_HYPNOSHROOM && !aPlant->mIsAsleep)
        {
            ...
            DieNoLoot();
            if (mZombieType != ZOMBIE_IMP)
            {
                Zombie* aImp = mBoard->AddZombieInRow(ZOMBIE_IMP, mRow, false);
                aImp->mPosX = mPosX;
                aImp->SetRow(mRow);
            }
        }
...
void Zombie::Draw(Graphics* g)
{
    ...
    if (!IsDeadOrDying() && mZombieType == ZombieType::ZOMBIE_BACKUP_DANCER && IsOnBoard())
    {
        float aCenterX = 50.0f;
        float aCenterY = aDrawPos.mBodyY + 40.0f;
        int aNumImages = 5;     
        float aRadius = 60.0f;     
        float aSpeed = 0.05f;    
        float aScale = 1.0f;    

        Image* aOrbitImage = Sexy::IMAGE_REANIM_DIAMOND; 
        ...
        if (aOrbitImage != nullptr)
        {
            ...
            {
                float aAngle = (mZombieAge * aSpeed) + (i * 2.0f * 3.14159f / aNumImages); // угол поворота
                ...
                Rect aSrcRect(0, 0, aOrbitImage->GetWidth(), aOrbitImage->GetHeight()); // берём информацию с картинки а именно ширину и высоту
                Rect aDestRect((int)aDrawX, (int)aDrawY, aScaledWidth, aScaledHeight); // X, Y, ширина, высота
...
void Zombie::EatPlant(Plant* thePlant)
{
        ...
        thePlant->mState == PlantState::STATE_SQUASH_PRE_LAUNCH)
    {
        if (!thePlant->mIsAsleep)
        {
            ...
            if (mZombieType == ZombieType::ZOMBIE_TRAFFIC_CONE)
            {
                Plant* aPlant = nullptr;
                while (mBoard->IteratePlants(aPlant))
                {
                    if (aPlant && aPlant->mRow == mRow)
                    {
                        if (mX - 210 <= aPlant->mX && mX + 30 >= aPlant->mX)
                        {
                            aPlant->mPlantHealth -= DAMAGE_PER_EAT * 2;
                            aPlant->mEatenFlashCountdown = std::max(aPlant->mEatenFlashCountdown, 25);
                            if (mPhaseCounter <= 0)
                            {
                                ...
                                mApp->AddTodParticle(mPosX, mPosY + 50, mRenderOrder + 2, ParticleEffect::PARTICLE_ZOMBIE_BOSS_FIREBALL);
                                mPhaseCounter = 120;
                            }
                        }
                    }
            ...
            {
                mBoard->mChallenge->GraveDangerSpawnGraveAt(thePlant->mPlantCol, thePlant->mRow);
                return;
            }
            else if (mZombieType == ZombieType::ZOMBIE_PAIL)
            {
                ...
                if (mPhaseCounter <= 0)
                {
                    mApp->AddTodParticle(aPosX, aPosY, mRenderOrder + 2, ParticleEffect::PARTICLE_ICEBALL_DEATH);
                    mPhaseCounter = 50;
                }
                while (mBoard->IteratePlants(aPlant))
                {
                    if (GetCircleRectOverlap(aPosX, aPosY, 115, aPlant->GetPlantRect()))
                    {
    ...
    else if (mZombieType == ZombieType::ZOMBIE_FOOTBALL)
    {
        ...
        while (mBoard->IteratePlants(aPlant))
        {
            if (GetCircleRectOverlap(aPosX, aPosY, 115, aPlant->GetPlantRect()))
            {
                ...
                {
                    mApp->PlayFoley(FoleyType::FOLEY_IGNITE);
                    mApp->AddTodParticle(aPosX, aPosY, mRenderOrder + 2, ParticleEffect::PARTICLE_FIREBALL_DEATH);
                    mPhaseCounter = 50;
                }
            }
        }
    ...
    else if (mZombieType == ZombieType::ZOMBIE_PAIL)
    {
        ...
        thePlant->mPlantHealth -= DAMAGE_PER_EAT / 4;
    }
    else
